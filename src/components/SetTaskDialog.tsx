...
export default function SetTaskDialog({ open, onClose, onApplied }: SetTaskDialogProps) {
  ...
  const [skillDialogState, setSkillDialogState] = useState<{
    largeId: string;
    smallId: string;
  } | null>(null);
    ...
    setTaskConfig((prev) => ({
      ...
      [largeId]: (prev[largeId] ?? []).map((s) =>
        s.category_small_id === smallId
          ? {
              ...s,
              skills: s.skills.map((sk) => (sk.id === skillRowId ? { ...sk, required_count: count } : sk)),
            }
          : s
      ),
    }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <IconButton onClick={handleRequestClose} aria-label="閉じる" size="small">
          <CloseIcon />
        </IconButton>
      </DialogTitle>
            ...
            <Box sx={{ p: { xs: 2, md: 4 } }}>
              ...
              >
                ...
                <Button
                  ...
                  sx={{ color: 'primary.light', borderColor: 'primary.light', alignSelf: { xs: 'stretch', sm: 'flex-end' } }}
                >
                  大カテゴリ追加
                </Button>
              </Stack>
                                    ...
                                    return (
                                      <Grid item xs={12} sm={6} md={4} key={sk.id}>
                                        ...
                                        >
                                          <CardContent>
                                            <Typography
                                              sx={{ fontSize: '16px', fontWeight: 700, mb: 1, color: 'primary.main' }}
                                            >
                                            ...
                                            <Stack direction="row" justifyContent="flex-end" spacing={0.5} sx={{ mt: 1.5 }}>
                                              {isEditing ? (
                                                <IconButton
                                                  ...
                                                  color="primary"
                                                  onClick={() => {
                                                    updateSkillCount(largeId, small.category_small_id, sk.id, editingCount);
                                              ...
                                              <IconButton
                                                size="small"
                                                onClick={() =>
                                                  removeSkill(largeId, small.category_small_id, sk.id)
                                                }
                                                sx={{ color: 'text.secondary' }}
                                              >
                                                <DeleteIcon fontSize="small" />
      ...
      >
        <Button variant="outlined" onClick={handleRequestClose} data-testid="set-task-cancel">
          キャンセル
        </Button>
        <Button
          variant="contained"
